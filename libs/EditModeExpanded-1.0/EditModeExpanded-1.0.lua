...
function lib:RegisterFrame(frame, name, db, anchorTo, anchorPoint, clamped)
    ...
    EditModeManagerExpandedFrame.AccountSettings[frame.system] = CreateFrame("CheckButton", nil,
        EditModeManagerExpandedFrame.AccountSettings, "UICheckButtonTemplate")
    ...
    if not lib.firstCheckButtonPlaced then
        ...
        checkButtonFrame:SetPoint("TOPLEFT", EditModeManagerExpandedFrame.AccountSettings.disableHighlightTexturesOption,
            "BOTTOMLEFT", 0, 10)
    else
        ...
        checkButtonFrame:SetPoint("TOPLEFT", EditModeManagerExpandedFrame.AccountSettings[previousSystemID], "BOTTOMLEFT",
            0, 10)
    end
...
function lib:RegisterCustomButton(frame, name, onClick)
    ...
    local button = CreateFrame("Button", nil, EditModeExpandedSystemSettingsDialog.Settings,
        "UIPanelButtonTemplate,ResizeLayoutFrame")
...
hooksecurefunc(f, "OnLoad", function()
    ...
    EditModeManagerExpandedFrame.Title = EditModeManagerExpandedFrame:CreateFontString(nil, "ARTWORK",
        "GameFontHighlightLarge")
    EditModeManagerExpandedFrame.Title:SetPoint("TOP", 0, -15)
    EditModeManagerExpandedFrame.Title:SetText("Expanded")
    EditModeManagerExpandedFrame.Border = EditModeManagerExpandedFrame.Border or
        CreateFrame("Frame", nil, EditModeManagerExpandedFrame, "DialogBorderTranslucentTemplate")
    EditModeManagerExpandedFrame.AccountSettings = EditModeManagerExpandedFrame.AccountSettings or
        CreateFrame("Frame", nil, EditModeManagerExpandedFrame)
    ...
    EditModeManagerExpandedFrame.CloseButton = EditModeManagerExpandedFrame.CloseButton or
        CreateFrame("Button", nil, EditModeManagerExpandedFrame, "UIPanelCloseButton")
    ...
    hooksecurefunc(EditModeManagerFrame, "MakeNewLayout",
        function(self, newLayoutInfo, layoutType, layoutName, isLayoutImported)
    ...
    --
    local frame = EditModeExpandedSystemSettingsDialog or
        CreateFrame("Frame", "EditModeExpandedSystemSettingsDialog", UIParent, "ResizeLayoutFrame")
    ...
    frame.Buttons.RevertChangesButton = frame.Buttons.RevertChangesButton or
        CreateFrame("Button", nil, frame.Buttons, "EditModeSystemSettingsDialogButtonTemplate")
    ...
    EditModeManagerExpandedFrame.AccountSettings.disableHighlightTexturesOption = EditModeManagerExpandedFrame
        .AccountSettings.disableHighlightTexturesOption or
        CreateFrame("CheckButton", nil, EditModeManagerExpandedFrame.AccountSettings, "UICheckButtonTemplate")
                ...
                for index, displayInfo in ipairs(systemSettingDisplayInfo) do
                    ...
                    if settingFrame then
                        ...
                        if displayInfo.setting == ENUM_EDITMODEACTIONBARSETTING_FRAMESIZE then
                            ...
                            settingFrame.cbrHandles:RegisterCallback(settingFrame.Slider,
                                MinimalSliderWithSteppersMixin.Event.OnValueChanged, OnValueChanged, settingFrame)
                        end
...
function getOffsetXY(frame, x, y)
    ...
    elseif anchorPoint == "TOPRIGHT" then
        ...
        return (x + width) - (((targetX + targetWidth) * parentscale) / scale),
            (y + height) - (((targetY + targetHeight) * parentscale) / scale)
    elseif anchorPoint == "CENTER" then
        local targetX, targetY, targetWidth, targetHeight = frame.EMEanchorTo:GetRect()
        local width, height = frame:GetSize()
        return (x + 0.5 * width) - (((targetX + 0.5 * targetWidth) * parentscale) / scale),
            (y + 0.5 * height) - (((targetY + 0.5 * targetHeight) * parentscale) / scale)
    ...
    elseif anchorPoint == "BOTTOM" then
        ...
        local width = frame:GetSize()
        return (x + 0.5 * width) - (((targetX + 0.5 * targetWidth) * parentscale) / scale),
            y - ((targetY * parentscale) / scale)
    elseif anchorPoint == "LEFT" then
        local targetX, targetY, _, targetHeight = frame.EMEanchorTo:GetRect()
        local _, height = frame:GetSize()
        return x - ((targetX * parentscale) / scale),
            (y + 0.5 * height) - (((targetY + 0.5 * targetHeight) * parentscale) / scale)
